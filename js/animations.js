...
(function () {
  ...
  function initParticles() {
    ...
    /* ── Lightweight Parallax Particle Layer ── */
    const layers = [
      {
        z: 1.0,
        spacing: 56,
    ...
    function animate() {
      ...
      layers.forEach(layer => {
        ...
        for (const dot of layer.dots) {
          ...
          if (isLight) {
            // Darker violet on light theme
            const l = 35 + normVal * 20;
            ctx.fillStyle = `hsla(272, 70%, ${l | 0}%, ${alpha.toFixed(3)})`;
          } else {
            // Neon purple glowing on dark theme
            const l = 55 + normVal * 35;
            const s = 80 + normVal * 20;
            ctx.fillStyle = `hsla(272, ${s | 0}%, ${l | 0}%, ${alpha.toFixed(3)})`;
          }
          ctx.fill();
        }
      });
      ...
      if (isLight) {
        spotG.addColorStop(0,   'rgba(168,85,247,0.08)');
        spotG.addColorStop(0.5, 'rgba(168,85,247,0.03)');
        spotG.addColorStop(1,   'rgba(168,85,247,0)');
      } else {
        spotG.addColorStop(0,   'rgba(192,132,252,0.18)');
        spotG.addColorStop(0.5, 'rgba(168,85,247,0.06)');
        spotG.addColorStop(1,   'rgba(168,85,247,0)');
      }
      ctx.fillStyle = spotG;
      ...
      if (isLight) {
        ...
        vG.addColorStop(0, 'rgba(10,7,20,0)');
        vG.addColorStop(1, 'rgba(6,4,14,0.70)');
      }
      ctx.fillStyle = vG;
